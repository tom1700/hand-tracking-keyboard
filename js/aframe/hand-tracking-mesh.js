...
AFRAME.registerSystem("hand-tracking-mesh", {
  ...
  updateReferenceSpace: function () {
    ...
    xrSession
      .requestReferenceSpace(refspace)
      .then(function (referenceSpace) {
        self.referenceSpace = referenceSpace.getOffsetReferenceSpace(
      ...
      if (!inputSource.hand) {
        ...
        for (const joint of this.hands[inputSource.handedness]) {
          ...
          if (inputSource.hand[joint.offset] !== null) {
            jointPose = frame.getJointPose(
              inputSource.hand[joint.offset],
              refSpace
            );
          }
          if (jointPose !== null) {
            joint.visible = true;
            joint.position = new THREE.Vector3(
